...
XUN 是一个基于 ![NoneBot](https://github.com/richardchien/nonebot) 和 ![酷Q](https://cqp.cc) 的功能性QQ机器人，目前提供了点播、音乐推荐、天气查询、识图、搜番、地震速报功能，由于是为了完成自己在群里的承诺，一时兴起才做的，所以写得比较粗糙，大家见谅。
...
* 计算: '计算', 'exp' [可使用任何基于python的语法，但要注意输出为所有变量的集合]
* 天气查询：'天气', '天气预报', '查天气'
* 识图：'image', '搜图', '识图' [已整合图信检功能]
* 识番剧: 'whatanime', '搜番', '识番'
* 音乐点播：'音乐', '点播', '来首', '给朕来首' [注意音乐名用《》或者标准格式 命令+空格，使用 歌名-歌手 可以更准确]
* 音乐推荐：'推荐音乐', '音乐推荐', '推荐一首'
* 地震速报(被动技能)：误差±10分钟 [注意只有国内地震震级>=4.0才会通报]
...
**需要注意的是加入了超时机制，如果 SauceNAO 和 ascii2d 其中一个在检索的时候超时则不会有对应的结果！**

### 计算

![2.png](https://i.loli.net/2020/01/04/4dM17YWtbTNGUXk.png)

任何使用 Python 来计算的公式都可以使用此功能来计算，会得到一个包含计算过程所有变量的值空间字典的回复，如果过程中所用的变量太多，需要快速得到结果，可以在最后赋值给一个特定的变量，比如: `end`，则会很容易在回复中找到结果。
